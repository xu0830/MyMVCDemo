...
(function () {
	...
	$(document).ready(function () {
		...
		j();	//	提交订单核对车票信息弹出层
		au();	//	disableClickSubmitButtonOrPreStepBUtton
		y();
		...
		if (ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.dc 
			|| ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.wc) {	//	单程或往返
			v();	//	发送ajax请求 confirmPassenger/getPassengerDTOs 
			F();
			ax()
		}
		$("#selected_ticket_passenger_all").prop("checked", true);
		h();
		if (ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc && $("#dw_fczk")[0]) { //	false
	...
	//	checkOrderInfo form数据中的passengerTicketStr字段
	getpassengerTickets = function () {
		var aA = "";
		for (var aB = 0; aB < limit_tickets.length; aB++) {
			var aC = limit_tickets[aB].seat_type + ",0," + limit_tickets[aB].ticket_type + "," + limit_tickets[aB].name + "," 
	...
	*/
	function Z(aM, aF, aG, aJ, aH, aD, aP, aO, aK, aA, aL, aE, aN, aI, aC, aB) {
		this.only_id = aM;	//default_0
		this.seat_type = aF;	//	""
		this.seat_type_name = aG;	//	""
		this.ticket_type = n(aI, aP, aJ);	//	"1"    n("", "", "")
		...
		*/
		...
		this.save_status = aL; // aL: checked='checked'
		this.tour_flag = aE; // dc
		this.isDisabled = aI == ticket_submit_order.ticket_type.student ? true : aN; // false
		this.isDefaultUsed = false;
		this.checkboxStatus = aC; // true
