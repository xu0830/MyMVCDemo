...
(function () {
	...
	function aN(cL) {
		...
		cL = a7(cL);
		var cN = ci(cL);	//是否包含D-动车
		if (cN) {//	true
			cL = cz(cL)
		}
		if ($("#avail_jf")[0].checked) {	//勾选 “显示积分兑还车次”
			cL = aU(cL)
		}
	...
	*/
	...
	function av(cL) {
		...
		if ($("#auto_query").is(":checked")) {	//	勾选自动查询 / false
		...
		$.ajax({
			...
			success: function (cP) {
				...
				if (cP.status) {
					...
					if (cP.data.flag == 1) {
						cP.data = cp(cP.data.result, cP.data.map)	//	解析result车票数据
					}
					trainListForIE = [];
	...
	function bv() {
		...
		var cN = br;
		var cR = [];
		if (cM.length > 0 || cS.length > 0 || filteredTrainArriveTime.length > 0 || ce.length > 0 
			|| cO.length > 0 || cQ.length > 0 || aJ.getComboText() != "" || $("#avail_ticket")[0].checked /*显示全部可预订车次*/) {
